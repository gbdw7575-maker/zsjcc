...
// ===== D5: 顶层 await 数据预取（要求父级 <Suspense> 包裹）=====
// 等 gameData 加载完成（App.vue onMounted 已发起，此处复用同一 Promise）
await gameData.load()

// 同步拉公告：传当前 season，后端返回该赛季 + 通用公告
let annList = []
try {
  const params = {}
...
const announcements = ref(annList)
...
// B6: 赛季切换后异步重拉公告（不影响 Suspense 已 resolve 的状态）
watch(() => gameData.currentSeason.value, async () => {
  try {
    const p = {}
    if (gameData.currentSeason.value) p.season = gameData.currentSeason.value
    const { data } = await announcementApi.getAnnouncements(p)
